    ...
    try:
        with os.scandir(folder_path) as entries:
            for entry in entries:
                ...
                if entry.endswith(".ab"):
                    list_of_AB_files.append(entry)
                elif entry.is_dir():
                    print(f"Directory: {entry.name}")
    except FileNotFoundError:
        print(f"The specified folder '{folder_path}' does not exist.")

backup_folder = "D:\\MITE_Cases\\bigtest_a70qxtc_20231126202631\\SM-A705MN-BACKUP-20231126202631\\Mite_Backup"

unpack_to_dis = backup_folder + "/UnpackedData"
tar_version = backup_folder + ".tar"
   
unpack_pull = backup_folder + "/UnpackedData"
pull_tar_version = backup_folder
    
try:
    for backed_up in list_of_AB_files:

...
preview_popups = st.empty()     #   use for notifications
big_img_list = []
big_vid_list = []
...
def video_to_frames(video_file, output_path):
    # Display information (assuming you are using Streamlit)
    ...
    try:
        ...
        # Get total frames and frames per second
        total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
        fps = int(cap.get(cv2.CAP_PROP_FPS))

        # Calculate seconds of video
        seconds_of_video = total_frames / fps

        # Calculate save interval to gather desired number of frames
...
#         cap = cv2.VideoCapture(video_file)
#         with preview_popups:
#             st.info("Creating collage for " + video_file)
#         total_frames= cap.get(7)
#         # print(total_frames)
#         fps = int(cap.get(cv2.CAP_PROP_FPS))
#         # print(fps)
#         seconds_of_video = total_frames / fps
#         # print(seconds_of_video)
#         save_interval = round(seconds_of_video / number_frames_to_collect)     #   gathers 4 frames
#         index = 0        
#         while cap.isOpened():
...
def makecollage(inputfolder, outputfolder):
    ...
    for path, subdirs, files in os.walk(inputfolder):
        for directory in subdirs:
            ...
            folder_Listing.append(path + "/" + directory)
    ...
    for subfolder in folder_Listing:
        ...
        try:
            ...
            for im in images:
                ...
                x_offset += im.size[0]
            collage_name = subfolder.split("/")
            collage_name = collage_name[-1:]
            collage_name = str(collage_name).strip("['")
            collage_name = (collage_name).split(".")
            collage_name = str(collage_name[0])
            collage_name = collage_name + "_MITE_Collage.jpg"
        # print(collage_name)
            new_im.save(outputfolder + "/" + collage_name)
        except ValueError:
            print("Thumbnails not generated for - ", subfolder)
            pass
        except cv2.error as e:
...
def gallery_view(folder):
        ...
        for f in file:
            ...
            try:
                ...
                if pictype != None:
                    ...
                    if img_size > 50000 and img_size <= 800000:
                        med_img.append(fil)
                    if img_size > 9000 and img_size <= 50000:
    ...
    # page_number = st.number_input("Page", 1, 1)
    
    # format = st.sidebar.selectbox("Choose Media Format", ["Images", "Videos"])

    if format == "Images":
        try:
            img_size_choice = st.sidebar.selectbox("Select Size Category", ["Large - " + str(len(large_imgs)), "Medium - " + str(len(med_img)), "Small - " + str(len(small_img))])
            ...
            list_of_img_chunks = [img_group[x:x+300] for x in range(0, len(img_group), 300)]            #Chunk size showing n images per page
            
            page_number = st.sidebar.number_input("Page of " + str(len(list_of_img_chunks)), 1, len(list_of_img_chunks))
            img_chunk = list_of_img_chunks[page_number - 1]                             #   takes user input and subs one to get list value ex 0 = 1

            # cols = cycle(st.columns(4))
            # for idx, img_chunk in enumerate(img_chunk):
            #     next(cols).image(img_chunk, width=150, caption=img_chunk[idx])
            idx = 0
...
find_folders(mite_folders)
try:
    unpack_folder = st.selectbox("Select a Case Backup", list_of_backups,format_func=lambda x: str(x.split("\\")[2]).strip("[]'"))
    # unpack_folder = st.selectbox("Select a Case Backup", list_of_backups)
    # current_case = str(unpack_folder.split("/")[-3:-2])
    current_case = str(unpack_folder.split("\\")[2])
    current_case = current_case.strip("[]'")
    st.header("Viewing:  " +current_case)
    if len(unpack_folder) == 0:
        with preview_popups:
            st.error("Select a case from the sidebar to preview")
except AttributeError:
    st.error("Select a Case Backup. This tool will only address backup directories formatted for MITE.")    #   These exceptions address when no case backups exist to read by the previewer
    pass
except NameError:
    pass
except TypeError:
...
try:
    ...
    if len(unpack_folder) > 0:
        ...
        if format == "Images":
            ...
            with preview_popups: 
                st.spinner("Preparing Image Previews")
                
                global video_thumb_folder
                video_thumb_folder = str(new_store_location.parent.absolute()) + "/VideoStills"
               
                global videocollagefolder
                videocollagefolder = str(new_store_location.parent.absolute()) + "/VideoCollages"
             
                if os.path.exists(video_thumb_folder) == False:
                    os.mkdir(video_thumb_folder)
                
                if os.path.exists(videocollagefolder) == False:
        ...
        if format == "Videos":
            video_gallery(str(new_store_location.parent.absolute()) + "\\VideoCollages")
            print("VIDEO COLLAGE FOLDER - "+ str(new_store_location.parent.absolute()) + "\\VideoCollages")
except TypeError:
    pass    #   to address no case backup data being present on 'NoneType' issues
